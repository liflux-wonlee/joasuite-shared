...
export function UserBadge() {
  ...
  const fallbackName =
    (data?.all ?? []).map((r) => (r.display_name || "").trim()).find((n) => n && !isEmailish(n)) || "";
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="min-w-0 leading-tight text-left rounded px-2 py-1 hover:bg-muted transition">
          ...
          {showEmail && <div className="text-[10px] font-normal text-muted-foreground/50 truncate">{email}</div>}
          {position && <div className="text-[10px] text-muted-foreground/60 truncate">{position}</div>}
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-56">
        <DropdownMenuLabel className="font-normal">
          {showName && <div className="text-sm font-medium truncate">{nameLine}</div>}
          {showEmail && <div className="text-xs text-muted-foreground truncate">{email}</div>}
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
        {ITEMS.map((it) => {
          const Icon = it.icon;
